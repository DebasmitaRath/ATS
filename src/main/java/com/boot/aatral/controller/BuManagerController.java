...
public class BuManagerController {
	...
	public ResponseEntity<BuManagerDto> updateUser(@RequestBody BuManagerDto buManagerDto,
			@PathVariable Integer requestResourceId) {
	...
	@GetMapping("/skillsByDivId/{divId}")
	public ResponseEntity<Map<Long, String>> loadSkills(@PathVariable Long divId) {
		Map<Long, String> skills = this.buManagerService.loadSkills(divId);
		return new ResponseEntity<Map<Long, String>>(skills, HttpStatus.OK);
	}
	
	
	@GetMapping("/getAllDivisions")
	public ResponseEntity<Map<Long, String>> loadDivisions() {
		Map<Long, String> skills = this.buManagerService.getAllDivisions();
		return new ResponseEntity<Map<Long, String>>(skills, HttpStatus.OK);
	}
